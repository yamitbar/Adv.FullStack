...
Click **Add location**. Start typing a full address (required) and pick one of the Geoapify suggestions — this silently captures the place's coordinates behind the scenes; there's still just one address field, no visible coordinates. Optionally add a custom title. Submit.

**6. Show the location on the map** (~30s)
Open **Map** from the navbar (or **View on map** from the trip page). Point out the new marker, click it to open the popup (title/address, trip name, visit date, a link back to Location Details), then use the trip filter dropdown to narrow the map to just this trip.

**7. Add a text memory** (~30s)
...
**10. Show the participant list** (~20s)
...
**11. Show the authorization difference** (~30s)
Still logged in as the second user, open a location or memory created by the first account — point out there is no Edit/Delete button, because creator-only controls simply don't render for a non-creator. Optionally show that the second user *can* add their own new memory to the same location, and that it appears as a marker on the shared trip's map too.

**12. Edit or delete a memory/location** (~30s)
Switch back to the first (creator) account. Edit a memory's text, or delete a location, and point out the confirmation prompt before deletion and the resulting empty/updated state. If you edit a location's address to a different place and re-select a suggestion, point out the map marker moves accordingly.

**13. Logout** (~15s)
Use the navbar logout control. Confirm you're returned to a logged-out view and that visiting a protected route like `/trips` (or `/map`) now redirects to `/login`.
